    ...
    let actor_chain                                         = SBSyncActor::wrap(16, move || {
        AtrSyncLambda::new_closure(
            Box::new(move |x: MsgNumberedMsg<MsgNumberedMsg<MsgRawBinary>>| {
                let mut block_json: serde_json::Value = serde_json::from_slice(x.msg.msg.value.as_slice()).expect("Unable to parse UTF8-string as JSON");
                // println!("Got blockjson : {}",block_json["blockHeight"].as_u64().unwrap());

                for tx in block_json["transactions"].as_array_mut().unwrap().iter() {
